...
public game() 
{
	try
	{
		rubashka = ImageIO.read(new File("D:\\Картинки покер наруто\\k0.png"));
	}
catch (Exception ex) {}
...
{
	if ((stopki[2].size()==13) &&(stopki[3].size()==13) &&(stopki[4].size()==13) &&(stopki[5].size()==13))
	{
		endGame = true;
		tmEndGame.start();
	}
...
private boolean testPerenos(int nom1, int nom2)
{
	...
	if ((nom2>=2)&&(nom2<=5))
	{
		if (nomKarti==(stopki[nom1].size()-1))
		{
			...
			else if ((getKarta2.tipKarta==12)&& (getKarta1.mast==getKarta2.mast)&&(getKarta1.tipKarta==0))
			{
				rez = true;
			}
...
private void load()
{
	for (int i=1;i<=52;i++)
	{
		stopki[0].add(new karta("D:\\Картинки покер наруто\\k"+(i)+".png", rubashka, i));
	}
}
